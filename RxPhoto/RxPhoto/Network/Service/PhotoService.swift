...
final class PhotoService {
    ...
    func fetchPhotoList(
        ...
        let urlPath = Environment.dev.baseURL + EndpointPath.listPhoto.path
    ...
    func searchPhotoList(
        ...
        let urlPath = Environment.dev.baseURL + EndpointPath.searchPhoto.path
        guard let parameter = try? request.toDictionary() else { return }
        let headers: HTTPHeaders = ["Authorization": "Client-ID \(Secrets.accessKey)"]
    ...
    func fetchSinglePhoto(
        ...
        let urlPath = Environment.dev.baseURL + EndpointPath.singlePhoto(id: id).path
        let headers: HTTPHeaders = ["Authorization": "Client-ID \(Secrets.accessKey)"]
